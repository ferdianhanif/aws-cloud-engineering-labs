# ☁️ Amazon EC2 Provisioning, Security Group Scoping & Workload Resilience

A hands-on cloud engineering lab project documenting the deployment of a resilient Amazon EC2 compute instance, automated server bootstrapping via User Data, network security scoping with Security Groups, vertical compute/storage scaling, and termination protection safeguards.
...
- **Automated Bootstrap**: Configured User Data script (`user_data_bootstrap.sh`) to automatically install Apache (`httpd`), enable it on boot, start the service, and serve a custom web page without manual SSH intervention.
...
.
├── README.md
├── user_data_bootstrap.sh
├── .gitignore
└── images/
    ├── 01_launch_config.png
    ├── 02_resized_t3small.png
    ├── 03_ebs_modified.png
    ├── 04_instance_running_public_ip.png
    └── 05_termination_protection_triggered.png
```
